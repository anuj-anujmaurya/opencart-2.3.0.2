...
final class Openbay {
	...
	public function encrypt($msg, $k, $base64 = false) {
		...
		$encrypted = openssl_encrypt($msg, 'aes-256-ctr', $k, OPENSSL_RAW_DATA, $iv);
	...
	public function decrypt($msg, $k, $base64 = false) {
		...
		$iv = substr($msg, 0, 16);
		$mac = substr($msg, -32);
		$encrypted = substr($msg, 16, -32);
	
		$calculated_mac = $this->pbkdf2($iv . $encrypted, $k, 1000, 32);
	
		if ($mac !== $calculated_mac) {
			return false;
		}
	
		$decrypted = openssl_decrypt($encrypted, 'aes-256-ctr', $k, OPENSSL_RAW_DATA, $iv);
	
		if ($decrypted === false) {
			return false;
		}
	
		$msg = unserialize($decrypted);
